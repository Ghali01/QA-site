function parse_tag_link(state, silent, options) {
    ...
    var totalContent = state.src.slice(state.pos, labelEnd + 1);
    var isMeta = totalContent.slice(0, 10) === "[meta-tag:";
    if (isMeta && !options.allowMetaTags) {
        return false;
    }
    var tagName = totalContent.slice(isMeta ? 10 : 5, -1);
    // check that the tag name follows specific rules TODO better docs
    var validationRegex = options.allowNonAscii
        ? /([a-z0-9\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF#+.-]+)/i
        : /([a-z0-9#+.-]+)/i;
    ...
    if (!silent) {
        var token = state.push("tag_link_open", "a", 1);
