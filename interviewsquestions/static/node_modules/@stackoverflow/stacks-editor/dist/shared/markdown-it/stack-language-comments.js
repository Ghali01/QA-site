function langCommentParser(matcher, tokenType, state, startLine, endLine, silent) {
    ...
    var matches = matcher.exec(lineText);
...
function sanitizeCodeBlockLangs(state) {
    ...
    for (var i = 0, len = state.tokens.length; i < len; i++) {
        var currentBlock = state.tokens[i];
        var nextBlock = state.tokens[i + 1];
    ...
    state.tokens = state.tokens.filter(function (t) { return !t.type.startsWith("stack_language"); });
    return false;
}
/**
