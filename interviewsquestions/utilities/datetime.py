...
def timeDaltaToInt(dalta):
	return dalta.days*24*60*60 +dalta.seconds

def getTimezoneName(tz_offset, common_only=True):
...
class TimezoneMiddleware:
    ...
    def __call__(self, request):
        tzOffset = request.COOKIES['utcOffset'] if 'utcOffset' in request.COOKIES else None 
        if tzOffset:
            timezone.activate(pytz.timezone(getTimezoneName(float(tzOffset))))
        else:
            timezone.deactivate()
